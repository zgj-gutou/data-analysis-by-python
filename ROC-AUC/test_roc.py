...
# 使用6折交叉验证，并且画ROC曲线
cv = StratifiedKFold(y, n_folds=6)   # 分成6份
classifier = svm.SVC(kernel='linear', probability=True,
                     random_state=random_state)  # 注意这里，probability=True,需要，不然预测的时候会出现异常。另外rbf核效果更好些。

mean_tpr = 0.0
mean_fpr = np.linspace(0, 1, 100)   # FPR在0-1之间分成100份，FPR在ROC曲线中是横坐标
# print(mean_fpr)
all_tpr = []
...
for i, (train, test) in enumerate(cv):
    ...
    # 通过roc_curve()函数，求出fpr和tpr，以及阈值thresholds。由ROC的定义也可知，一个阈值对应一个fpr和tpr
    # 这里参数y[test]表示测试集的label,probas_[:, 1]表示为label为1的可能性（概率）
